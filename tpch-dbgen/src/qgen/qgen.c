...
#endif
#ifdef _WIN32
#include "win_defines.h" /* Defines matching the make file */
#include "process.h"
#endif /* WIN32 */
#include <ctype.h>
#include <time.h>
    ...
    if (snum >= 0)
        if (optind < ac)
            for (i=optind; i < ac; i++)
                {
                char qname[10];
                sprintf(qname, "%ld", (long)SEQUENCE(snum, atoi(av[i])));
                qsub(qname, flags);
                }
        else
            for (i=1; i <= QUERIES_PER_SET; i++)
                {
                char qname[10];
                sprintf(qname, "%ld", (long)SEQUENCE(snum, i));
                qsub(qname, flags);
                }
    else
        if (optind < ac)
